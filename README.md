...
The official local-only path starts from the published CLI package. Bootstrap prepares the local runtime and built-in example Hotline; status and run-example then verify the same machine can complete one local self-call.
...
For contributor or source installs, use the repository root with `npm install` and the root CLI entry:
