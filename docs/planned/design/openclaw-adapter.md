...
对 OpenClaw 来说，理想目标不是让模型自己学会 Caller 协议步骤，而是：

1. OpenClaw 只看到一个或少量稳定 skill
2. skill 背后由 Caller 侧桥接服务完成协议编排
...
这意味着 OpenClaw 天然更适合调用一个“高层能力接口”，而不是直接调 Caller Controller 的多步接口。

如果直接把 Caller Controller 暴露给 OpenClaw，会出现三个问题：
...
- hotline 列表会变，skill 名称不应频繁变化
- 通用 skill 更适合先查 catalog 再选 hotline

### 3.2 在 skill 内部支持 alias

如果确实有高频能力，可以在桥接层内部给常用 hotline 做 alias，例如：

- `classify-text -> foxlab.text.classifier.v1`
- `extract-fields -> owlworks.data.extractor.v1`

但 alias 应属于桥接层内部映射，不应替代协议里的 `hotline_id` 真相层。
...
- `baseUrl` 推荐指向 Caller 侧 skill adapter，而不是直接指向 Platform 或 Responder
- `apiKey` 推荐由 Caller 身份或 Caller 专用桥接层身份提供
- 不建议把 Responder API key 暴露给 OpenClaw
...
- 供 OpenClaw 查询当前可用 remote hotline
...
本文这一节描述的是 OpenClaw bridge 对 Caller Controller 的底层桥接方式，不是终端用户通过统一 `ops` 客户端使用系统的主路径。

当前仓库里，OpenClaw bridge 最适合基于 [packages/caller-controller-core/src/index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-hotline-protocol/packages/caller-controller-core/src/index.js) 或 [apps/caller-controller/src/server.js](/Users/hejiajiudeeyu/Documents/Projects/remote-hotline-protocol/apps/caller-controller/src/server.js) 实现。

一次 `invoke` 的内部流程建议固定为：

1. `GET /controller/hotlines`
...
- `POST /controller/inbox/pull` 只在当前 `L0 local transport` 单机联调中由 Caller 侧主动触发。
- 真正远端部署时，Responder Runtime 应自己消费其 inbox；OpenClaw bridge 不应长期依赖这个步骤。
...
- 当任务需要远程能力时，优先使用 `remote-hotline` skill
- 不要自行拼 Caller Controller 的多步 HTTP 调用
...
- “用 remote hotline `foxlab.text.classifier.v1` 处理这条工单。”
- “把这段材料交给远程字段抽取 skill。”
...
不要把 Caller Controller 的原始全量响应直接暴露给模型，否则模型会被过多协议噪音干扰。
...
### 8.3 不要把 skill 名称绑定到 responder

OpenClaw 适配时，skill 名称应围绕能力或通用入口，而不是围绕 responder 身份命名。
...
- Caller 调哪个 `hotline_id`

而不是把 Responder 身份暴露成用户心智主入口。
...
- `OpenClaw 可适配`
- `适配点已经清晰`
- `但仍需在 Caller 侧补一层 OpenClaw-friendly skill bridge`

如果后续要实现，建议把它放在单独目录，例如：

- `apps/openclaw-bridge`

而不要把 OpenClaw 专有语义直接写进 Caller Controller。
