...
本指南覆盖 `platform`、`buyer`、`seller` 的受支持部署形态。
...
- 终端用户机器上的 `buyer` 和 `seller`：在 npm 正式发布完成前，优先仓库内 `npm run ops -- ...`
...
- `deploy/ops`：终端用户包，buyer 常驻，seller 为可选本地角色
- `deploy/relay`：共享 transport relay
- `deploy/buyer`：独立 buyer controller（默认 SQLite）
- `deploy/seller`：面向运维与 CI 的独立 seller controller
- `deploy/all-in-one`：本地集成栈

Profile 意图：

- `deploy/platform` 面向生产，不默认启用 bootstrap demo sellers
...
3. 若尚未审批，先审批 seller 和 subagent，再重跑 `npm run ops -- bootstrap`
...
`ops-console` 提供 setup wizard，覆盖 buyer 注册、官方示例安装、review 提交、seller 启用和本地自调用。  
`ops-console` 还支持本地口令解锁流，敏感凭据保存在 `~/.delexec/secrets.enc.json`，而非浏览器存储。  
`enable-seller` 仅启用本地 seller 运行时；平台 review 控制目录可见性与远程可用性，不阻止本地运行时启动。
...
- 完整运维 bootstrap 流程见 [public-stack-operator-guide.md](/Users/hejiajiudeeyu/Documents/Projects/remote-subagent-protocol/docs/current/guides/public-stack-operator-guide.md)
...
Relay 是部署模式下 buyer 与 seller controller 之间共享的 transport 运行时。

对终端用户 ops 客户端，relay 默认由本地 supervisor 启动和管理。  
独立 relay 部署主要用于 CI、本地集成与高级运维场景。

- Buyer 与 seller 均要求 `TRANSPORT_BASE_URL`
...
- EmailEngine 适配器：[index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-subagent-protocol/packages/transports/emailengine/src/index.js)
- Gmail 适配器：[index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-subagent-protocol/packages/transports/gmail/src/index.js)
- 共享邮件 envelope helper：[index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-subagent-protocol/packages/transports/email/src/index.js)
...
- 推荐升级路径：当 buyer 需要跨容器替换或接入外部运维工具时设置 `DATABASE_URL`
...
本地演示可选 seller signing，但非演示部署应视为必需。
...
将 `platform` 与 `seller` 分开部署时，请在两侧使用相同 seller 身份和密钥对。  
面向生产的 `deploy/platform` 默认应关闭 bootstrap sellers，除非你刻意运行预接线演示环境。

## 部署建议

- `platform`：作为服务端镜像发布并部署，搭配托管 PostgreSQL
- `public-stack`：需要单一公网运维包时优先
- `buyer`：既支持容器部署也支持直接嵌入；标准化运维场景优先 Docker
- `seller`：终端用户机器优先 `npm run ops -- ...`，运维管理的独立服务场景使用容器部署
