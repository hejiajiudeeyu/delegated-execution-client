...
- `caller` and `responder` on an end-user machine: prefer the repo-local `npm run ops -- ...` path until npm publish is explicitly completed
...
- `deploy/ops`: end-user package with caller always on and responder as an opt-in local role
- `deploy/relay`: shared transport relay
- `deploy/caller`: standalone caller controller, SQLite by default
- `deploy/responder`: standalone responder controller deployment profile for operators and CI
- `deploy/all-in-one`: local integration stack

Profile intent:

- `deploy/platform` is production-oriented and does not enable bootstrap demo responders by default
...
The relay is the shared transport runtime between caller and responder controllers in deployment mode.
...
- Caller and responder both require `TRANSPORT_BASE_URL`
...
- EmailEngine adapter: [index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-hotline-protocol/packages/transports/emailengine/src/index.js)
- Gmail adapter: [index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-hotline-protocol/packages/transports/gmail/src/index.js)
- shared email envelope helpers: [index.js](/Users/hejiajiudeeyu/Documents/Projects/remote-hotline-protocol/packages/transports/email/src/index.js)
...
- Recommended upgrade path: set `DATABASE_URL` when caller must survive container replacement or run with external operations tooling
