...
import {
  ...
  evaluateTimeouts,
  loadCallerConfig
} from "@delexec/caller-controller-core";
...
describe("applyResultPackage", () => {
  it("returns SUCCEEDED for valid ok result without signature", () => {
    ...
    expect(result.eventType).toBe("caller.request.succeeded");
  ...
  it("returns UNVERIFIED for context mismatch (wrong responder_id)", () => {
    const request = makeRequest({ status: "ACKED" });
    const body = {
      request_id: "req_test",
      result_version: "0.1.0",
      responder_id: "responder_wrong",
      hotline_id: "agent.v1",
      status: "ok",
      output: {}
    };
  ...
  it("returns UNVERIFIED for context mismatch (wrong hotline_id)", () => {
    const request = makeRequest({ status: "ACKED" });
    const body = {
      request_id: "req_test",
      result_version: "0.1.0",
      responder_id: "responder_1",
      hotline_id: "wrong.agent",
      status: "ok",
      output: {}
    };
  ...
  it("allows null responder_id on request (wildcard match)", () => {
    const request = makeRequest({ status: "ACKED", responder_id: null });
    const body = {
      request_id: "req_test",
      result_version: "0.1.0",
      responder_id: "any_responder",
      hotline_id: "agent.v1",
      status: "ok",
      output: { summary: "ok" },
      schema_valid: true
  ...
  it("allows null hotline_id on request (wildcard match)", () => {
    const request = makeRequest({ status: "ACKED", hotline_id: null });
    const body = {
      request_id: "req_test",
      result_version: "0.1.0",
      responder_id: "responder_1",
      hotline_id: "any.agent",
      status: "ok",
      output: { summary: "ok" },
      schema_valid: true
    };
...
describe("createRequestRecord", () => {
  ...
  it("falls back to config for ack_deadline_s", () => {
    const config = { ...loadCallerConfig(), ack_deadline_s: 200 };
    const record = createRequestRecord(config, {});
