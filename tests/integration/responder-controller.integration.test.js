...
describe("responder-controller integration", () => {
  ...
  beforeAll(async () => {
    server = createResponderControllerServer({ serviceName: "responder-controller-test" });
  ...
  it("queues and completes success task", async () => {
    const created = await jsonRequest(baseUrl, "/controller/tasks", {
      method: "POST",
      body: {
        request_id: "req_responder_success_1",
        simulate: "success",
        delay_ms: 30
      }
    });
  ...
  it("returns error package for token_expired simulation", async () => {
    const created = await jsonRequest(baseUrl, "/controller/tasks", {
      method: "POST",
      body: {
        request_id: "req_responder_token_expired_1",
  ...
  it("supports replay only after result is ready", async () => {
    const created = await jsonRequest(baseUrl, "/controller/tasks", {
      method: "POST",
      body: {
        request_id: "req_responder_replay_1",
        simulate: "success",
        delay_ms: 60
      }
    });
  ...
  it("registers responder identities through responder-controller", async () => {
    ...
    try {
      ...
      expect(registered.body.responder_id).toBe("responder_register_test");
      expect(registered.body.hotline_id).toBe("register.test.v1");
      expect(registered.body.api_key).toMatch(/^sk_responder_/);
      expect(registered.body.review_status).toBe("pending");

      const catalog = await jsonRequest(platformUrl, "/v2/hotlines?capability=contract.extract");
      expect(catalog.status).toBe(200);
      expect(catalog.body.items.some((item) => item.hotline_id === "register.test.v1")).toBe(false);

      const approved = await jsonRequest(platformUrl, "/v2/admin/hotlines/register.test.v1/approve", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${platformState.adminApiKey}`
        },
        body: { reason: "approved for responder runtime" }
      });
      expect(approved.status).toBe(200);

      const approveResponder = await jsonRequest(platformUrl, "/v2/admin/responders/responder_register_test/approve", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${platformState.adminApiKey}`
        },
        body: { reason: "responder approved for runtime" }
      });
      expect(approveResponder.status).toBe(200);

      const enabledCatalog = await jsonRequest(platformUrl, "/v2/hotlines?capability=contract.extract");
      expect(enabledCatalog.status).toBe(200);
      expect(enabledCatalog.body.items.some((item) => item.hotline_id === "register.test.v1")).toBe(true);
    } finally {
  ...
  it("registers through responder-controller with a caller api key and stores the issued responder credentials", async () => {
    ...
    const caller = await jsonRequest(platformUrl, "/v1/users/register", {
      method: "POST",
      body: { contact_email: "responder-owner@test.local" }
    });
    ...
    const state = createResponderState({
      responderId: "responder_owned_test",
      hotlineIds: ["owned.test.v1"]
    });
    const responderServer = createResponderControllerServer({
      serviceName: "responder-controller-register-owned-test",
  ...
  it("executes tasks through a configured process adapter", async () => {
    const scriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "responder-process-adapter-"));
    ...
    const processServer = createResponderControllerServer({
      serviceName: "responder-controller-process-adapter-test",
      state: createResponderState({
        responderId: "responder_process_adapter",
        hotlineIds: ["process.adapter.v1"],
        hotlines: [
          {
            hotline_id: "process.adapter.v1",
      ...
      executor: createHotlineRouterExecutor([
        {
          hotline_id: "process.adapter.v1",
          display_name: "Process Adapter",
          enabled: true,
  ...
  it("executes tasks through a configured http adapter", async () => {
    ...
    const httpServer = createResponderControllerServer({
      serviceName: "responder-controller-http-adapter-test",
      state: createResponderState({
        responderId: "responder_http_adapter",
        hotlineIds: ["http.adapter.v1"],
        hotlines: [
          {
            hotline_id: "http.adapter.v1",
  ...
  it("introspects inbound token, auto-acks platform, and sends result to caller queue", async () => {
    ...
    const autoResponderServer = createResponderControllerServer({
      serviceName: "responder-controller-auto-test",
      transport,
      platform: {
        baseUrl: platformUrl,
    ...
    const autoResponderUrl = await listenServer(autoResponderServer);

    try {
      const registered = await jsonRequest(platformUrl, "/v1/users/register", {
        method: "POST",
        body: { contact_email: "responder-auto@test.local" }
      });
      const authHeader = { Authorization: `Bearer ${registered.body.api_key}` };
      const requestId = "req_responder_auto_1";
      ...
      const pulled = await jsonRequest(autoResponderUrl, "/controller/inbox/pull", {
        method: "POST",
        body: {}
      });
      ...
      const callerQueue = await waitFor(async () => {
        const polled = await callerTransport.peek();
      ...
      expect(callerQueue.items[0].request_id).toBe(requestId);
    } finally {
      await closeServer(autoResponderServer);
      await closeServer(platformServer);
    }
  });
  ...
  it("reports FAILED event to platform when execution returns error", async () => {
    ...
    const callerTransport = createLocalTransportAdapter({ hub, receiver: "caller-controller" });
    const responderServer = createResponderControllerServer({
      serviceName: "responder-controller-failed-event-test",
      transport,
      platform: {
        baseUrl: platformUrl,
        apiKey: responder.api_key,
    ...
    try {
      const registered = await jsonRequest(platformUrl, "/v1/users/register", {
        method: "POST",
        body: { contact_email: "responder-failed@test.local" }
      });
      const authHeader = { Authorization: `Bearer ${registered.body.api_key}` };
      const requestId = "req_responder_failed_event_1";
      ...
      const callerQueue = await waitFor(async () => {
        const polled = await callerTransport.peek();
        if (polled.items.length === 0) {
          throw new Error("caller_failed_result_not_ready");
        }
        return polled;
      });
      expect(callerQueue.items[0].result_package.status).toBe("error");
    } finally {
      await closeServer(responderServer);
      await closeServer(platformServer);
    }
  ...
  it("supports injected executors for local processing facilities", async () => {
    ...
    const responderState = createResponderState({
      responderId: "responder_runtime_custom",
      hotlineIds: ["runtime.custom.v1"]
    });
    const responderTransport = createLocalTransportAdapter({ hub, receiver: "responder_runtime_custom" });
    const callerTransport = createLocalTransportAdapter({ hub, receiver: "caller-controller" });
    const customExecutor = createFunctionExecutor(async ({ requestId, taskType, taskInput, payload, hotlineId }) => ({
      status: "ok",
      output: {
        request_id: requestId,
    ...
    const customServer = createResponderControllerServer({
      serviceName: "responder-controller-custom-executor-test",
    ...
    try {
      await responderTransport.send({
        message_id: "msg_custom_exec_1",
        from: "caller-controller",
        to: "responder_runtime_custom",
        request_id: "req_custom_exec_1",
        responder_id: "responder_runtime_custom",
        hotline_id: "runtime.custom.v1",
        task_type: "extract",
      ...
      expect(callerQueue.items[0].result_package.status).toBe("ok");
      expect(callerQueue.items[0].result_package.output).toEqual({
        request_id: "req_custom_exec_1",
        handled_by: "custom-function-executor",
        hotline_id: "runtime.custom.v1",
        task_type: "extract",
      ...
      expect(callerQueue.items[0].result_package.usage).toEqual({
        tokens_in: 3,
        tokens_out: 2
      });
    } finally {
  ...
  it("dedupes repeated request_id and replays completed result", async () => {
    ...
    const responderState = createResponderState({
      ...
      hotlineIds: ["foxlab.text.classifier.v1"]
    });
    const responderTransport = createLocalTransportAdapter({ hub, receiver: "responder_foxlab" });
    const callerTransport = createLocalTransportAdapter({ hub, receiver: "caller-controller" });
    const dedupeServer = createResponderControllerServer({
      serviceName: "responder-controller-dedupe-test",
      state: responderState,
      transport: responderTransport
    });
    ...
    try {
      await responderTransport.send({
        ...
        from: "caller-controller",
        to: "responder_foxlab",
        request_id: "req_responder_dedupe_1",
      ...
      expect(responderState.tasks.size).toBe(1);
      ...
      expect(replayedQueue.items[1].result_package.request_id).toBe("req_responder_dedupe_1");
  ...
  it("applies guardrails and reports responder metrics", async () => {
    ...
    const callerTransport = createLocalTransportAdapter({ hub, receiver: "caller-controller" });
    const guardedServer = createResponderControllerServer({
      serviceName: "responder-controller-guardrail-test",
      transport: responderTransport,
      platform: {
        baseUrl: platformUrl,
        apiKey: responder.api_key,
    ...
    try {
      const registered = await jsonRequest(platformUrl, "/v1/users/register", {
        method: "POST",
        body: { contact_email: "responder-guardrail@test.local" }
      });
      const authHeader = { Authorization: `Bearer ${registered.body.api_key}` };
      const requestId = "req_responder_guardrail_1";
      ...
      expect(callerQueue.items[0].result_package.status).toBe("error");
      expect(callerQueue.items[0].result_package.error.code).toBe("CONTRACT_TIMEOUT_EXCEEDS_RESPONDER_LIMIT");
      ...
      expect(metrics.body.by_type["responder.task.received"]).toBeGreaterThanOrEqual(1);
      expect(metrics.body.by_type["responder.task.rejected"]).toBeGreaterThanOrEqual(1);
    } finally {
  ...
  it("starts heartbeat loop and updates platform availability", async () => {
    ...
    try {
      await waitFor(async () => {
        ...
        const catalog = await jsonRequest(platformUrl, "/v2/hotlines");
        const item = catalog.body.items.find((candidate) => candidate.hotline_id === responder.hotline_id);
  ...
  it("runs queued tasks concurrently when worker concurrency is greater than one", async () => {
    ...
    const concurrentState = createResponderState({
      responderId: "responder_concurrent",
      hotlineIds: ["responder.concurrent.v1"],
      workerConcurrency: 2
    });
    const concurrentServer = createResponderControllerServer({
      serviceName: "responder-controller-concurrency-test",
    ...
    try {
      ...
      for (const requestId of ["req_responder_concurrent_1", "req_responder_concurrent_2", "req_responder_concurrent_3"]) {
        const accepted = await jsonRequest(concurrentUrl, "/controller/tasks", {
          method: "POST",
          body: {
            request_id: requestId,
            responder_id: "responder_concurrent",
            hotline_id: "responder.concurrent.v1",
            delay_ms: 10
          }
        });
