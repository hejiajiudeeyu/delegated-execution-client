...
const ALL_PACKAGES = [
  ...
  ...CLI_PACKAGES.map((item) => item.workspace)
];
...
async function verifyLibraryPackage(item, installDir) {
  ...
  console.log(`[check-client-packages] ok library=${item.workspace}`);
}
...
async function verifyServicePackage(item, installDir) {
  ...
  try {
    ...
    console.log(`[check-client-packages] ok service=${item.workspace} bin=${item.bin}`);
  } catch (error) {
...
async function verifyCliPackage(item, installDir) {
  const packageRoot = packageInstallPath(installDir, item.workspace);
  const binPath = resolveBinEntry(packageRoot, item.bin);
  const cliEnv = {
    ...process.env,
  ...
  await execFileAsync(process.execPath, [binPath, "setup"], {
    cwd: installDir,
    env: cliEnv
  });
  ...
  if (!output) {
    throw new Error(`cli_output_missing:${item.workspace}`);
  }
  const parsed = JSON.parse(output);
  if (typeof parsed.ok !== "boolean") {
    throw new Error(`cli_output_invalid:${item.workspace}`);
  }
  console.log(`[check-client-packages] ok cli=${item.workspace} bin=${item.bin}`);
}

async function main() {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "client-pack-check-"));
  ...
  try {
    ...
    for (const item of CLI_PACKAGES) {
      await verifyCliPackage(item, installDir);
    }
  } finally {
...
main().catch((error) => {
  console.error(`[check-client-packages] ${error.stack || error.message}`);
  process.exit(1);
});
