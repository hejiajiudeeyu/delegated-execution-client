...
export * from "@delexec/caller-controller-core";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const ROOT_DIR = path.resolve(__dirname, "../../..");

loadEnvFiles(buildOpsEnvSearchPaths(ROOT_DIR, "caller"));
...
function loadPlatformConfigFromEnv() {
  ...
  return {
    baseUrl,
    apiKey: process.env.CALLER_PLATFORM_API_KEY || process.env.PLATFORM_API_KEY || null
  };
}
  ...
  if (transportType === "email") {
    ...
    if (provider === "emailengine") {
      return createEmailEngineTransportAdapter({
        ...
        sender: process.env.TRANSPORT_EMAIL_SENDER || process.env.CALLER_CONTACT_EMAIL || null,
...
if (isDirectRun()) {
  ...
  const serviceName = process.env.SERVICE_NAME || "caller-controller";
  ...
  if (persistence) {
    hydrateCallerState(state, await persistence.loadSnapshot());
  }
  const server = createCallerControllerServer({
    ...
    background: {
      ...
      receiver: process.env.TRANSPORT_RECEIVER || serviceName,
      inboxPollIntervalMs: Number(process.env.CALLER_CONTROLLER_INBOX_POLL_INTERVAL_MS || 250),
      eventsSyncIntervalMs: Number(process.env.CALLER_CONTROLLER_EVENTS_SYNC_INTERVAL_MS || 250)
    },
    onStateChanged: persistence
      ? async (currentState) => {
          await persistence.saveSnapshot(serializeCallerState(currentState));
        }
      : null
  });
  ...
  server.on("close", () => {
    if (persistence) {
      void persistence.saveSnapshot(serializeCallerState(state));
